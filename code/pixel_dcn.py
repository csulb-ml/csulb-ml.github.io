...
def conv2d(inputs, num_outputs, kernel_size, scope, d_format='NHWC'):
    outputs = tf.contrib.layers.conv2d(
        ...
        data_format=d_format, activation_fn=None, weights_initializer=tf.contrib.layers.xavier_initializer(), biases_initializer=None, reuse=tf.AUTO_REUSE)
    return outputs
...
def shift_tensor(inputs, axis, row_shift, column_shift, scope):
    if row_shift:
        ...
        rows = tf.add(rows[row_shift:], [row_zeros]*row_shift)
    ...
    if column_shift:
        ...
        columns = tf.add(columns[column_shift:], [columns_zeros]*column_shift)
